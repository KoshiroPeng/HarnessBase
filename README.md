...
HernessDemo 是一个面向中小企业的在线项目管理 Web 产品。当前仓库的主线目标不是继续扩展“交付平台化”能力，而是先完成登录、项目、任务、成员与权限这些核心协作场景的第一阶段 MVP。

当前仓库已经具备后端工程骨架、基础架构约束、评审清单、测试规范、API 与错误码文档，以及一套可继续复用的发布与可观测性材料；真正还需要重点推进的是 `web/` 前端工程与前后端业务闭环。

命名上，文档统一使用 `HernessDemo`；脚本、服务名和制品中的 `herness-demo` 暂视为既有技术标识保留。
