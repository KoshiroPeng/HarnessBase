...
- A04：自动发现旧单体、旧包名或旧路径误用
- A05：自动发现 workflow 指向不存在的源码路径
- A06-A08：自动提醒 SQL、API、响应码、发布材料的同步关系

## 推荐推进顺序

1. 维护并持续验证 A01-A03 文档护栏。
2. 优先补 A05 workflow 路径存在性检查，避免发布和 CI 路径回退。
3. 再补 A04 历史事实误用扫描，先以提醒模式运行。
4. 最后补 A06-A08 API、响应码、SQL、发布材料的同步提醒。
...
该命令当前应输出 A01/A02/A03 通过。若失败，先修复元数据、相对链接或锚点，再继续做后端、前端或发布相关验证。
