...
本文档只记录当前仓库已经落地的真实技术事实，作为开发、测试、评审、发布和文档治理的共同基线。
...
自动化治理主线以文档护栏、路径护栏和同步提醒为主，入口见 [docs/plans/automation-delivery-map.md](../plans/automation-delivery-map.md)。
...
以下说法只允许出现在历史纠偏语境，不允许继续作为当前项目事实：

- 当前系统仍是 `RuoYi-Vue-Plus` 单体后台
- 当前启动入口是 `ruoyi-admin`
- 当前存在 `ruoyi-extend`
- 当前 SQL 目录是 `server/script/sql`
- 当前前端使用 Vue 3、Vite、Pinia、TypeScript、Element Plus、pnpm
...
- 自动化接入由 [docs/plans/automation-delivery-map.md](../plans/automation-delivery-map.md) 固定
