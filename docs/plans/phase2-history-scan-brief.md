...
本文档用于说明 HarnessBase 第二阶段自动化检查，也就是历史事实误用扫描，后续应如何接入本地脚本或 CI。
...
第二阶段最合理的接法，是先把历史事实误用扫描作为非阻断提醒接入 CI，先稳定命中语境，再考虑是否升级为更严格门禁。
