...
status: active
owner: "@PengKang"
description: ProjectPilot 计划文档目录入口，汇总当前迭代、后续待办与任务状态模板。
---
...
本目录沉淀 ProjectPilot 的当前迭代计划和后续待办，帮助协作者理解项目现在在做什么、下一步准备做什么。

当前阶段的计划主线已经明确调整为 Web 产品研发优先，发布治理与平台化建设作为后续支撑方向保留。
...
- 如果要系统理解 Harness Engineering 在当前仓库中的方法边界和能力组成，补充阅读 [docs/architecture/harness-engineering-reference.md](../architecture/harness-engineering-reference.md)。
- 如果要统一记录任务执行状态，优先使用 [docs/plans/task-status-template.md](task-status-template.md)。
- 做需求评审时，用本目录确认需求是否已进入当前迭代或 backlog。
- 做开发前评估时，用本目录确认当前优先级和边界，避免偏离当前阶段目标。
- 做文档治理时，用本目录检查新的规则或设计是否已经进入可执行计划。
