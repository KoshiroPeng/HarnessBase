...
- 跟进 [docs/reference/README.md](../reference/README.md) 中记录的前后端接口差异，按 [docs/plans/frontend-backend-api-drift-fix-brief.md](frontend-backend-api-drift-fix-brief.md) 优先处理 `/workflow/definition/definitionXml/{definitionId}` 与 `/workflow/definition/xmlString/{id}` 的不一致。

## P2：代码与质量硬化

- 清理或补齐前端 API 客户端中后端不存在的接口封装，继续优先处理 `web/src/api/workflow/definition/index.ts` 的 `definitionXml` 残留。
