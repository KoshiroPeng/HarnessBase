...
- 持续扫描直接跨层调用；裸 HTTP 客户端首轮核对已完成，当前命中集中在社会化登录 provider 适配器和基础设施层，未发现业务 Controller / Service 直接发起外部 HTTP 请求。`javax.*` 首轮核对已完成，当前仅剩合法的 `javax.sql.DataSource` 标准 JDBC 用法，未发现需要迁移到 `jakarta.*` 的旧 EE 残留。
