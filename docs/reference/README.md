...
| `web/src/api/monitor/cache/index.ts` 中仍有 `/monitor/cache/getNames`、`/getKeys`、`/getValue`、`/clearCacheName`、`/clearCacheKey`、`/clearCacheAll` | 后端 [CacheController.java](../../server/ruoyi-modules/ruoyi-system/src/main/java/org/dromara/system/controller/monitor/CacheController.java) 当前只暴露 `/monitor/cache` | 后续只改文档时不得把这些缓存细分接口写入 API 摘要；代码修复任务应决定删除前端残留或补齐后端缓存管理接口 |
