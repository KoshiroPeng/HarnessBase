...
status: active
owner: "@PengKang"
description: ProjectPilot 评审输出模板目录入口，汇总各阶段评审与验证证据模板。
---
...
本目录提供 ProjectPilot 在需求评审、设计评审、代码评审和测试用例评审中的统一输出模板，帮助 AI 与人工评审都按同一结构记录结论、问题和后续动作。
...
- 当前已补齐前端设计评审和前端代码评审模板，可直接用于后续 `web/` 主线落地。
...
- 若评审或验证涉及 JDK 17 / Spring Boot 3 迁移、模块化单体重构或前端结构调整，建议在模板中显式写出“目标基线是否满足”和“仍存在哪些历史残留”。
