...
| [docker/sentinel-dashboard.Dockerfile](docker/sentinel-dashboard.Dockerfile) | Sentinel Dashboard 镜像构建文件 |
| [docker/http-healthcheck.sh](docker/http-healthcheck.sh) | 通用 HTTP 健康检查脚本 |
...
3. 若使用源码构建镜像，先确认宿主机已具备 JDK 17、Maven、Node.js、npm，并确认磁盘空间足以容纳 Maven、npm 和 Docker 构建缓存。
4. 若要启动 `ruoyi-sentinel`，需要先把可用的 `sentinel-dashboard.jar` 放到 [deploy/compose/docker](docker) 目录下，文件名必须是 `sentinel-dashboard.jar`。当前仓库默认不会提交该 Jar，避免把来源不明或体积较大的二进制直接入库。
5. 若使用默认 `.env.example` 或 `.env.prod.example`，必须在正式环境先复制为 `.env` 并替换敏感值，尤其是：
   - `MYSQL_ROOT_PASSWORD`
   - `NACOS_AUTH_TOKEN`
   - 对外暴露端口
   - `FILE_UPLOAD_PATH`
6. 若通过 GitHub Actions 远端部署 compose，建议把 `APP_DEPLOY_DIR` 配成独立目录，例如 `/opt/harness-base-compose`，不要与单服务 SSH 发布目录共用。
...
- Sentinel 控制台 `http://127.0.0.1:${SENTINEL_DASHBOARD_PORT}`
- Nacos 控制台 `http://127.0.0.1:${NACOS_CONSOLE_PORT}`
...
- `NACOS_HTTP_PORT` 映射的是 Nacos Server API `8848`，主要供微服务注册发现和配置读取使用。
- `NACOS_CONSOLE_PORT` 映射的是 Nacos Console `8080`，浏览器访问控制台时应使用这个端口，而不是 `/nacos` 路径。
- `SENTINEL_DASHBOARD_PORT` 映射的是 Sentinel Dashboard `8718`，若依菜单中的“Sentinel控制台”应访问这个端口。
- Sentinel Dashboard 当前由本地提供的 `sentinel-dashboard.jar` 构建镜像。如果目标环境没有该 Jar，`ruoyi-sentinel` 镜像将无法完成构建。
...
- 再分别检查：
  ...
  - `http://127.0.0.1:${SENTINEL_DASHBOARD_PORT}`
  - `http://127.0.0.1:${NACOS_CONSOLE_PORT}`
- 如果某个服务仍处于 `starting`，优先看该服务日志，而不是立即判断整套部署失败。

### `ruoyi-sentinel` 构建失败，提示找不到 `sentinel-dashboard.jar`

- 先确认 [deploy/compose/docker](docker) 目录下存在 `sentinel-dashboard.jar`。
- 当前仓库默认通过 `.gitignore` 忽略该文件，不会把本地下载的 Dashboard 二进制直接提交到仓库。
- 如果你是在新机器或新环境首次执行 compose，需要先自行准备该 Jar，再执行 `bash deploy/compose/manage-compose.sh up-apps` 或 `build-images`。
